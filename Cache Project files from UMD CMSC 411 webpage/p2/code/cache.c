    ...
    for (int k = 0; k < (cache_split ? 2 : 1); k++) {
        ...
        for (i = 0; i < cache_to_init->n_sets; i++) {
            ...
            for (j = 0; j < cache_to_init->associativity; j++) {
                Pcache_line new_line = (Pcache_line) calloc(1,sizeof(cache_line));
                if (new_line == NULL) {
                ...
                new_line->LRU_prev = (Pcache_line)NULL;
                new_line->LRU_next = (Pcache_line)NULL;
...
void flush()
{
    ...
    // We must loop through both I and D caches if split is enabled
    for (int k = 0; k < (cache_split ? 2 : 1); k++) {
